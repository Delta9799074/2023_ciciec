...
extern void MBX_CH0_IRQHandler(void);
extern void MBX_CH1_IRQHandler(void);

extern void IOPMP_IRQHandler(void);
...
const sg_mbx_ch0_config[CONFIG_MBX_CH0_NUM] = {
    {MBX_CH0_BASE, MBX_CH0_IRQn, MBX_CH0_IRQHandler},
};

int32_t target_get_mbx_ch0_count(void)
{
    return CONFIG_MBX_CH0_NUM;
}

int32_t target_get_mbx_ch0(int32_t idx, uint32_t *base, uint32_t *irq, void **handler)
{

    if (idx >= target_get_mbx_ch0_count()) {
        return -1;
    }

    if (base != NULL) {
        *base = sg_mbx_ch0_config[idx].base;
    }

    if (irq != NULL) {
        *irq = sg_mbx_ch0_config[idx].irq;
    }

    if (handler != NULL) {
       *handler = (void*) MBX_CH0_IRQHandler;
    }
...
const sg_mbx_ch1_config[CONFIG_MBX_CH1_NUM] = {
    {MBX_CH1_BASE, MBX_CH1_IRQn, MBX_CH1_IRQHandler},
};

int32_t target_get_mbx_ch1_count(void)
{
    return CONFIG_MBX_CH1_NUM;
}

int32_t target_get_mbx_ch1(int32_t idx, uint32_t *base, uint32_t *irq, void **handler)
{

    if (idx >= target_get_mbx_ch1_count()) {
        return -1;
    }

    if (base != NULL) {
        *base = sg_mbx_ch1_config[idx].base;
    }

    if (irq != NULL) {
        *irq = sg_mbx_ch1_config[idx].irq;
    }

    if (handler != NULL) {
       *handler = (void*) MBX_CH1_IRQHandler;
    }
...
const sg_iopmp_config[CONFIG_IOPMP_NUM] = {
    {IOPMP_BASE, IOPMP_IRQn, IOPMP_IRQHandler},
};

int32_t target_get_iopmp_count(void)
{
    return CONFIG_IOPMP_NUM;
}

int32_t target_get_iopmp(int32_t idx, uint32_t *base, uint32_t *irq, void **handler)
{

    if (idx >= target_get_iopmp_count()) {
        return -1;
    }

    if (base != NULL) {
        *base = sg_iopmp_config[idx].base;
    }

    if (irq != NULL) {
        *irq = sg_iopmp_config[idx].irq;
    }

    if (handler != NULL) {
       *handler = (void*) IOPMP_IRQHandler;
    }
