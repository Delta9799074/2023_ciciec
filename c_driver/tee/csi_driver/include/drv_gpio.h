...
typedef void *gpio_pin_handle_t;   //无类型指针，可以指向任何类型的数据
...
*/
gpio_pin_handle_t csi_gpio_pin_initialize(int32_t gpio_pin, gpio_event_cb_t cb_event);   //函数嵌套：gpio_event_cb_t cb_event是一个函数，等价于static void gpio_interrupt_handler(int32_t idx)
...
/**
  \brief       config pin direction  配置引脚方向
  \param[in]   pin       gpio pin handle to operate.
