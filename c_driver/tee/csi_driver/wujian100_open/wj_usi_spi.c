...
#define TRANSFER_STAT_TRAN      3
    ...
    uint32_t tot_num;
} wj_usi_spi_priv_t;  //结构体变量分配内存空间；结构体的每个成员相对结构体首地址的偏移量都是最大基本类型成员字节大小的整数倍
...
/**
  \brief use gpio pin to simulate ss line for hardware controlled Output mode. 硬件控制，使用gpio电平高低去控制 cs 信号
*/
...
int32_t drv_usi_spi_config_datawidth(spi_handle_t handle, uint32_t datawidth)
{
    ...
    if ((datawidth >= (WJENUM_SPI_DATASIZE_4 + 1)) && (datawidth <= (WJENUM_SPI_DATASIZE_16  + 1))) {  // datawidth >= 4 && datawidth <= 16
        addr->USI_SPI_CTRL &= WJ_USI_SPI_CTRL_DATA_SIZE;  //datasize = 0xfff0
...
/**
  ...
  \return      error code
*/
int32_t drv_usi_spi_config_baudrate(spi_handle_t handle, uint32_t baud)  //0x0000313a
{
    SPI_NULL_PARAM_CHK(handle);  //好像进入了 0x00003158 之后没有跳出来，往下执行 
    ...
    int32_t sckdv = drv_get_usi_freq(spi_priv->idx) / baud;  //20000000/1000000=20  pc=0x0000316e jal pc=0x0000447a

    if (sckdv < 0x1000000) { 
        addr->USI_CLK_DIV0 =  sckdv;  //设置的是 SCK 时钟，Fpclk/20 = 1mhz  sckdv = 0x14 =20  有执行
    } else {
        return ERR_SPI(DRV_ERROR_PARAMETER);
    }

    spi_priv->state |= SPI_CONFIGURED; //1<<2 100 = 4 
...
static int32_t wj_spi_set_polarity(wj_usi_reg_t *addr, WJENUM_SPI_POLARITY polarity)
{
    ...
    switch (polarity) {
        case WJENUM_SPI_CLOCK_POLARITY_LOW:
            addr->USI_SPI_CTRL &= (~WJ_USI_SPI_CTRL_CPOL);  //1 << 7 = 8'b10000000  ~ == 8'b01111111   bit7=1'b0  CPOL
            break;

        case WJENUM_SPI_CLOCK_POLARITY_HIGH:
            addr->USI_SPI_CTRL |= WJ_USI_SPI_CTRL_CPOL; //1 << 7 = 8'b10000000 == 8'b10000000  bit7=1'b1  
            break;
//仿真得到 spi_ctrl = 0x017 = 00010111  bit7 bit6 = 00 ，因为 spi_ctrl 复位值 = 0x07，所以这两句话不会改变 spi_ctrl值
        default:
            return ERR_SPI(DRV_ERROR_PARAMETER);
    }
...
static int32_t wj_spi_set_phase(wj_usi_reg_t *addr, WJENUM_SPI_PHASE phase)
{
    switch (phase) {
        case WJENUM_SPI_CLOCK_PHASE_MIDDLE:
            addr->USI_SPI_CTRL &= (~WJ_USI_SPI_CTRL_CPHA); //1 << 6 = 8'b01000000  ~ == 8'b10111111   bit6=1'b0  CPHA
            break;  //j 0x000031dc

        case WJENUM_SPI_CLOCK_PHASE_START:
            addr->USI_SPI_CTRL |= WJ_USI_SPI_CTRL_CPHA; //1 << 6 = 8'b01000000    bit6=1'b1
            break;
...
int32_t drv_usi_spi_config_format(spi_handle_t handle, spi_format_e format)
{
    ...
    switch (format) {
        case SPI_FORMAT_CPOL0_CPHA0:
            polarity = WJENUM_SPI_CLOCK_POLARITY_LOW;  //初始时钟=0
            phase = WJENUM_SPI_CLOCK_PHASE_MIDDLE;   //在第二个时钟变化沿改变data in 
            break;

        case SPI_FORMAT_CPOL0_CPHA1:
            polarity = WJENUM_SPI_CLOCK_POLARITY_LOW; //初始时钟=1
            phase = WJENUM_SPI_CLOCK_PHASE_START;    //在第一个时钟变化沿改变data in 
            break;
...
int32_t drv_usi_spi_config_mode(spi_handle_t handle, spi_mode_e mode)
{
    ...
    if (mode == SPI_MODE_MASTER) {
        spi_priv->state |= SPI_CONFIGURED;
        addr->USI_SPI_MODE |= WJ_USI_SPI_MODE_MASTER;   ///设为 SPI MASTER 工作模式
        return 0;
...
int32_t drv_usi_spi_config_ss_mode(spi_handle_t handle, spi_ss_mode_e ss_mode)
{
    ...
    switch (ss_mode) {  
        case SPI_SS_MASTER_SW:   //选中，SPI_SS_MASTER_SW
            spi_priv->ss_mode = SPI_SS_MASTER_SW;  //master software 控制
            break;

        case SPI_SS_MASTER_HW_OUTPUT: //pc=0x00003322 检查是否执行了 wj_spi_ss_init
...
{
    uint32_t base = 0u;
    uint32_t irq = 0u;  //开辟变量空间值
    void *handler;
    uint32_t ssel = 0u;

    int32_t ret = target_usi_spi_init(idx, &base, &irq, &handler, &ssel);  //spi 设备 赋值基地址，中断向量号，中断处理函数等 pc = 0x00003ffc
    ...
    wj_usi_spi_priv_t *spi_priv = &spi_instance[idx];  //pc = 0x0000357e
    ...
    addr->USI_MODE_SEL = USI_MODE_SPI;  //0x2 选择 USI 设备下的 SPI device  pc = 0x000035b8
    addr->USI_SPI_MODE = WJ_USI_SPI_MODE_MASTER;   /* spi master mode 0x1*/

    drv_irq_register(spi_priv->irq, handler);  //pc = 0x00002d48 jump
    drv_irq_enable(spi_priv->irq);  //pc = 0x00002d14
...
/**
  ...
  \return      error code
*/
int32_t drv_usi_spi_config(spi_handle_t handle,   //0x0000362e
                           int32_t          baud,
                           spi_mode_e       mode,
                           spi_format_e     format,
                           ...
                           int32_t          bit_width)
{
    SPI_NULL_PARAM_CHK(handle);  //检测 SPI 设备初始化 是否完成
    wj_usi_spi_priv_t *spi_priv = handle;  //任意类型指针 handle 赋值给 spi_priv结构体

    if ((spi_priv->state & SPI_INITIALIZED) == 0U) {  //没有初始完，一开始 state = uint8'b0
        return ERR_SPI(DRV_ERROR_UNSUPPORTED);
    }

    if (spi_priv->status.busy) {  //处于busy状态
        return ERR_SPI(DRV_ERROR_BUSY);
    }
    ...
    wj_usi_reg_t *addr = (wj_usi_reg_t *)(spi_priv->base);  //base uint32_t
    addr->USI_EN = 0;  //USI 相关寄存器

    int32_t ret = 0;

    if (baud >= 0) {
        ret = drv_usi_spi_config_baudrate(handle, baud);  //0x00003678  
    ...
    if (mode >= 0) { 
        ret = drv_usi_spi_config_mode(handle, mode);  //0x0000322e
    ...
    if (format >= 0) {   //应该修改为 format>= 0 
        ret = drv_usi_spi_config_format(handle, format);  //0x000031a0

        if (ret < 0) {
            return ret;
        }
        // printf("format success!!!");  //理论上这句会打印
    }

    if (order > 0) {
        ret = drv_usi_spi_config_bit_order(handle, order);  //0x00003282  这里没有执行
    ...
    if (ss_mode >= 0) {  //怎么好像是这句话先执行的？
        ret = drv_usi_spi_config_ss_mode(handle, ss_mode); //0x000032be 

        if (ret < 0) {
            return ret;
        }
        // printf("ss_mode success!!!");  
    }

    if (bit_width >= 0) {
        ret = drv_usi_spi_config_datawidth(handle, bit_width);  //0x000030f6 这里没有执行
    ...
    addr->USI_EN    = 3;  //enable usi module and enable function module 
    spi_priv->state |= SPI_CONFIGURED;
...
/**
  ...
  \return      error code
*/
int32_t drv_usi_spi_send(spi_handle_t handle, const void *data, uint32_t num) //data： 指向数据buffer的指针

{
    ...
    uint8_t *ptxbuffer = (uint8_t *)data;  //待发送的数据所指向的存储空间

    spi_priv->send_num      = num;  //等待发送的数据列 1

    uint32_t once_len, i;
    addr->USI_EN    = 0xf;
    // wj_spi_ss_control(spi_priv, SPI_SS_ACTIVE);  //选中，开始往tx_buffer中传输数据

    while (spi_priv->send_num) {  //只有等待发送num = 0 循环才会结束
        int timeout = 1000000;
        once_len = (spi_priv->send_num >= USI_TX_MAX_FIFO) ?  //待发送的数据items >= MAXFIFO=0x10， once_len=0x10 TX_FIFO中规定只存放2 bytes 
                   USI_TX_MAX_FIFO : spi_priv->send_num;

        for (i = 0; i < once_len; i++) {
            addr->USI_TX_RX_FIFO = *ptxbuffer++;   //将待发送的数据存储进入 TX_buffer中，之后硬件上检测到非空，就进行发送
        }

        while ((addr->USI_SPI_STA == 0x1) && (timeout--));  //RO 只读，说明会自动判断，1表示 SPI设备没有在工作，已经发送完buffer中的16 bits 数据（也就是send 会先等待接收端对之前发送数据的确认）

        spi_priv->send_num -= once_len;
    }

    // wj_spi_ss_control(spi_priv, SPI_SS_INACTIVE); //pc=0x00003854
    spi_priv->status.busy   = 0U;
    addr->USI_EN = 0;
