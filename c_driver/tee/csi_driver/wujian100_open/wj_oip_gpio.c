...
gpio_pin_handle_t csi_gpio_pin_initialize(int32_t gpio_pin, gpio_event_cb_t cb_event)
{
    ...
    for (i = 0; i < CONFIG_GPIO_NUM; i++) {
        csi_gpio_port_initialize(i);  //i=0，初始化0号 port 端口在IO空间映射有寄存器，包含控制端口引脚输入输出的寄存器，控制端口引脚复用的寄存器等
    }  

    /* obtain the gpio pin information */
    uint32_t port_idx;
    int32_t pin_idx = target_gpio_pin_init(gpio_pin, &port_idx);  //obtain the pin idx= return gpio_pin 
