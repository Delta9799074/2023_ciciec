...
/**
  ...
  \return      error code
*/
int32_t csi_spi_uninitialize(spi_handle_t handle)  //pc = 0x00003eda
{
    return drv_usi_spi_uninitialize(handle);  //pc = 0x00003554
}
...
int32_t csi_spi_config(spi_handle_t handle,
                       ...
                       int32_t          bit_width)
{
    return drv_usi_spi_config(handle, baud, mode, format, order, ss_mode, bit_width);  //pc=0x00003d2e
}
...
\brief      receiving data from SPI receiver. if non-blocking mode, this function only start the receiving,
            ...
            if blocking mode, this function return after operation completes or error happens.
\param[in]  handle spi handle to operate.
\param[out] data  Pointer to buffer for data to receive from SPI receiver 直接从buffer中读取数据
\param[in]  num   Number of data items to receive  接收5个 byte数据
\return     error code
*/
